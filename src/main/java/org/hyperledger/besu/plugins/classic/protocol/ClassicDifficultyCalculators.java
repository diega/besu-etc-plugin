...
/** Difficulty calculators for Ethereum Classic network hardforks. */
public abstract class ClassicDifficultyCalculators {
  ...
  private ClassicDifficultyCalculators() {}

  /** Difficulty calculator with the difficulty bomb paused (DieHard hardfork). */
  public static final DifficultyCalculator DIFFICULTY_BOMB_PAUSED =
      (time, parent) -> {
  ...
  /** Difficulty calculator with the difficulty bomb delayed (Gotham hardfork). */
  public static final DifficultyCalculator DIFFICULTY_BOMB_DELAYED =
      (time, parent) -> {
        final BigInteger parentDifficulty = difficulty(parent.getDifficulty());
        final BigInteger difficulty =
            ensureMinimumDifficulty(
  ...
  /** Difficulty calculator with the difficulty bomb removed (DefuseDifficultyBomb hardfork). */
  public static final DifficultyCalculator DIFFICULTY_BOMB_REMOVED =
      (time, parent) -> {
        final BigInteger parentDifficulty = difficulty(parent.getDifficulty());
        return ensureMinimumDifficulty(
            BigInteger.valueOf(Math.max(1 - (time - parent.getTimestamp()) / 10, -99L))
                .multiply(parentDifficulty.divide(DIFFICULTY_BOUND_DIVISOR))
                .add(parentDifficulty));
      };

  /** Difficulty calculator using EIP-100 style with uncle adjustment (Atlantis and later). */
  public static final DifficultyCalculator EIP100 =
      (time, parent) -> {
        final BigInteger parentDifficulty = difficulty(parent.getDifficulty());
